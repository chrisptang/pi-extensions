...
function goalModeRules(goalLabel: string) {
	return [
		...
		"- When progress depends on information or a decision only the user can supply — a credential, an environment-specific or business-specific value, or a choice between valid alternatives — state the question plainly in your response, then call goal_wait with that question as the reason and no resume_after_ms. The user's reply wakes the goal. Prefer asking over guessing once the answer is not discoverable from the worktree, and never keep retrying tools to avoid asking.",
