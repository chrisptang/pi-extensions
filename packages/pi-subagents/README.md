...
A collapsed completion shows the job's agent, model, state, elapsed time, turns, cache rate, token totals, and cost; the cost is display-only and never reaches the model.
