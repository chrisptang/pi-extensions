---
"@chrisptang/pi-compaction-prompt": minor
---

Add `@chrisptang/pi-compaction-prompt`, which injects a Markdown compaction prompt into Pi's context summarization. The prompt is read from `<workspace>/.pi/compaction.md` when the project is trusted and otherwise from `~/.pi/agent/compaction.md`, so a repository can override the user-wide policy.
