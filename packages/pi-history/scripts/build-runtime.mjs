...
// The editor subclass is imported only when a TUI session seeds history, so it
// must stay behind a lazy chunk rather than loading with the extension.
const FORBIDDEN_EAGER_INPUTS = ["editor.ts"];
...
export async function buildRuntime({
	...
	const stagingDirectory = await mkdtemp(
		join(dirname(resolvedOutputDirectory), ".pi-history-dist-"),
	);
...
async function assertSafeOutputDirectory(outputDirectory) {
	...
	const isTestOutput = firstSegment?.startsWith(".pi-history-build-test-");
	if (
		relativeOutput === "" ||
		relativeOutput === ".." ||
